...
impl<const N: usize> AEADCipherCodec<N> {
    pub fn new(cipher: CipherKind, password: &[u8]) -> Result<Self, base64ct::Error> {
        match cipher {
    ...
    fn encode<CM: CipherMethod + KeyInit>(&mut self, context: &mut Context, item: BytesMut, dst: &mut BytesMut) -> anyhow::Result<()> {
        if self.kind.is_aead_2022() {
            ...
            dst.put(&header[..]);
            udp::new_encoder::<CM>(&self.key, 0, nonce)?.encode_packet(temp, dst).map_err(|e| anyhow!(e))?;
        } else {
    ...
    fn decode<CM: CipherMethod + KeyInit>(&mut self, context: &mut Context, src: &mut BytesMut) -> anyhow::Result<Option<BytesMut>> {
        if src.is_empty() {
            return Ok(None);
        }
    ...
    fn decode0<CM: CipherMethod + KeyInit>(&self, context: &mut Context, src: &mut BytesMut) -> anyhow::Result<BytesMut> {
        if self.kind.is_aead_2022() {
            let nonce_length = udp::nonce_length(self.kind).map_err(anyhow::Error::msg)?;
            let tag_size = self.kind.tag_size();
            ...
            let mut packet = udp::new_decoder::<CM>(&self.key, server_session_id, nonce)?.decode_packet(src).map_err(|e| anyhow!(e))?.unwrap();
