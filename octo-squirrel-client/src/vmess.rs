...
pub(crate) async fn transfer_tcp(
    ...
    let client_to_server = async {
        ...
        let mut wof = FramedWrite::new(wo, ClientAEADCodec::new(&header, session.clone()));
    ...
    let server_to_client = async {
        let mut rof = FramedRead::new(ro, ClientAEADCodec::new(&header, session.clone()));
...
impl ClientAEADCodec<'_> {
    pub fn new(header: &RequestHeader, session: Arc<Mutex<ClientSession>>) -> ClientAEADCodec {
    ...
    fn encode(&mut self, mut item: BytesMut, dst: &mut BytesMut) -> Result<(), Self::Error> {
        if let None = self.body_encoder {
            let mut buffer = BytesMut::new();
            {
                ...
                buffer.put_slice(&session.request_body_iv().lock().unwrap());
                buffer.put_slice(session.request_body_key());
                buffer.put_u8(session.response_header());
            }
            ...
            self.body_encoder = Some(AEADBodyCodec::encoder(&self.header, self.session.clone()));
        }
...
impl Decoder for ClientAEADCodec<'_> {
    ...
    fn decode(&mut self, mut src: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        if let None = self.body_decoder {
            {
                let session = self.session.lock().unwrap();
                let header_length_cipher = Aes128GcmCipher::new(&KDF::kdf16(session.response_body_key(), vec![KDF::SALT_AEAD_RESP_HEADER_LEN_KEY]));
        ...
        if self.header.command == RequestCommand::UDP {
            return self.body_decoder.as_mut().unwrap().decode_packet(src);
        } else {
            return self.body_decoder.as_mut().unwrap().decode_payload(src);
        }
    }
}
